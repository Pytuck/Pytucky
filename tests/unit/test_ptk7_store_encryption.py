...
def test_store_roundtrip_with_supported_encryption_modes(
    ...
    store = Store(
        file_path,
        options=BinaryBackendOptions(encryption=encryption, password=password),
        open_existing=False,
    )
...
def test_store_rejects_missing_or_wrong_password(tmp_path: Path, level: str) -> None:
    ...
    writer = Store(
        file_path,
        options=BinaryBackendOptions(encryption=level, password="secret123"),
        open_existing=False,
    )
    ...
    with pytest.raises(EncryptionError, match="需要提供密码"):
        Store(file_path, options=BinaryBackendOptions())

    with pytest.raises(EncryptionError, match="密码错误"):
        Store(file_path, options=BinaryBackendOptions(password="wrong-password"))
...
def test_store_flush_preserves_loaded_encryption_level_when_only_password_is_provided(tmp_path: Path) -> None:
    ...
    writer = Store(
        file_path,
        options=BinaryBackendOptions(encryption="medium", password="secret123"),
        open_existing=False,
    )
    writer.create_table("users", [Column(int, name="id", primary_key=True), Column(str, name="name")])
    writer.insert("users", {"name": "Alice"})
    ...
    reopened = Store(file_path, options=BinaryBackendOptions(password="secret123"))
    reopened.update("users", 1, {"name": "Bob"})
    reopened.flush()
    reopened.close()

    assert b"Bob" not in file_path.read_bytes()
    verified = Store(file_path, options=BinaryBackendOptions(password="secret123"))
...
def test_store_rejects_invalid_encryption_level_before_writing(tmp_path: Path) -> None:
    store = Store(
        ...
        options=BinaryBackendOptions(encryption="legacy", password="secret123"),
        open_existing=False,
    )
    store.create_table("users", [Column(int, name="id", primary_key=True), Column(str, name="name")])
