...
| `save` | 10.76ms |
| `query_pk` | 1.45ms |
| `query_indexed` | 1.51ms |
| `load` | 1.71ms |
| `reopen` | 1.46ms |
| `reopen_first_query` | 45.10μs |
| `file_size` | 56.0KB |
...
|------|-----------|---------------|------|
| 插入 | 646.36ms | 834.38ms | 快约 **22.5%** |
| 索引查询（100 次） | 1.42ms | 1.88ms | 快约 **24.7%** |
| 保存 | 492.17ms | 562.03ms | 快约 **12.4%** |
| 加载 | 140.72ms | 337.28ms | 快约 **58.3%** |
| reopen | 139.03ms | 321.98ms | 快约 **56.8%** |
| 文件大小 | 5.98MB | 6.09MB | 小约 **1.8%** |

### 补充观察

- `query_pk` 现在是 **1.45ms / 100 次**。这项在旧 `pytuck` 表格里没有单独列，但对当前 ORM 实际体验非常关键。
- `reopen_first_query` 现在是 **77.91μs**。旧 `pytuck` 文档正文里提过“首次懒查询约 `121.6μs`”，如果仅作近似参考，当前 PTK7 也更快。
- 相比本仓库前一轮文档中的旧结果，当前 PTK7 的变化主要集中在读路径：
  - `query_pk`：从 1.51ms 降到 1.45ms
  - `query_indexed`：从 1.48ms 降到 1.42ms
  - `reopen`：从 141.58ms 降到 139.03ms
  - `reopen_first_query`：从 84.45μs 降到 77.91μs
...
因此 `save` 已从早期文档中的 881.11ms 降到当前约 492ms，并稳定反超 `pytuck` 文档基线的 562.03ms。
...
也就是说，当前版本已经达到“在保留基本 ORM 用法前提下，单文件 PTK7 主路径整体领先旧基线”的目标。
