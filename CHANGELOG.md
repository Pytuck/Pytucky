...
- 修复 lazy `SortedIndexProxy` 在更新后仍返回旧范围主键的问题。
- 修复 Session 无法把 nullable 字段更新为 `None` 的问题。
- 让批量插入、批量更新和表结构事务回滚保持异常原子性。
- 为 PTK7 落盘增加轻量原子替换和失败临时文件清理，数据库本体仍保持单文件。
- 加固 PTK7 header、schema、表块、行与索引的边界校验，并增加随机损坏文件测试。
- 扩充现有 benchmark，记录内存快照事务的启动、回滚和峰值内存。
- HMAC 改为固定分块流式计算，避免认证时复制完整数据库内容。
- benchmark 增加单条更新 flush、加密 reopen、认证峰值内存和 `.tmp` 残留指标。
- benchmark 的 `--count` 现在要求为正整数，命令行和程序化入口使用相同校验。
- nullable 字段参与范围比较时按“不匹配”处理，不再抛出 `TypeError`。
- 加密 PTK7 文件新增兼容 pytuck 的文件级 HMAC 认证标签，并提供严格认证模式。
- 补充 MIT 许可证文件，收紧完整类型标注门禁并校正发布版本说明。
